...
class Ajax extends MY_Controller
{
    ...
    private function gameFutureRole(): array {
        ...
        $roles = $this->game
            ...
            ->setWithMordred($withMordred)
            ->setWithOberon($withOberon)
            ->setWithPerceval($withPerceval)
            ->setWithMorgana($withMorgana)
     ...
     * @return array
     */
    private function playerVoteForTeam(): array {
        ...
        $this->currentPlayer->voteForTeam($gameUid, $teamUid);
     ...
     * @return array
     */
    private function playerVoteForTeamCancel(): array {
        $gameUid = $this->currentGame->getGameUid();
     ...
     * @return array
     */
    private function playerVoteForQuest(): array {
        $this->load->model('player_model', 'player');
        $gameUid = $this->currentGame->getGameUid();
        $playerUid = $this->input->post('playerUid');
        $this->player->init($playerUid);
     ...
     * @return array
     */
    private function playerVoteForQuestCancel(): array {
        $gameUid = $this->currentGame->getGameUid();
        $this->currentPlayer->cancelVote($gameUid);
